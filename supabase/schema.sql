...
create policy char_skills_read on public.character_skills for select using (auth.role() = 'authenticated');
create policy char_skills_admin_write on public.character_skills for all using (public.is_admin()) with check (public.is_admin());
