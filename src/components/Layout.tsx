...
export function Layout() {
  ...
  function exportCsv() {
    ...
    // Members
    lines.push(['House', 'Name', 'Player', 'Rank', 'Function', 'Military Role', 'Noble', 'Status', 'Attending', 'HP', 'MP', 'Resource', 'Coven', 'Tithe Paid', 'Tithe Notes', 'Notes'].map(q).join(','));
    lance.data.members.forEach(m => {
      const house = lance.data.houses.find(h => h.id === m.house_id)?.name ?? 'Unassigned';
      lines.push([house, m.name, m.player_name ?? '', m.rank ?? '', m.function ?? '', m.military_function ?? '', m.is_noble ? 'Y' : 'N', m.status, m.attending_event ? 'Y' : 'N', m.hp ?? '', m.mp ?? '', m.resource ?? '', m.coven ?? '', m.tithe_paid ? 'Y' : 'N', m.tithe_notes ?? '', m.notes ?? ''].map(q).join(','));
    });
